...
Contains the entire PSR-4 source tree for the NV oOS Content Graph — AI addon — the composition root (`Plugin`), the `nvoos/core` engine bridge (`CoreBridge`), adapters, embeddings, RAG, agent memory, REST endpoints, 13 AI-powered tools, and the ported engine pieces (Wave E6: OOS shadow runner, gates, suppression, wave-1 service bridges; markup elicitation subsystem under `Engine/Markup/`; Paper Store flat-file knowledge store under `Engine/PaperStore/`; OKF knowledge engine under `Engine/Okf/`; Crawl4AI background coordinator under `Engine/Crawler/`).
...
| `NvoosContentGraphAi\Engine\OosServiceBridges` | `Engine/OosServiceBridges.php` | Wave-1 OOS service resolvers — semantic compressor, data budget, erlang C, error/cost tracking (Wave E6, standalone-only) |
